...
const Ioss = () => {
  ...
  useEffect(() => {
    // HEAD PART 
    axios.get(`/api/v1/ioss/text/IOSS`)
      .then((res) => {
        setIossTop(res?.data)
      })
    ...
    axios.get(`/api/v1/ioss/question/IOSS`)
      .then((res) => {
    ...
    // QUESTION WITH IMAGE 
    axios.get(`/api/v1/ioss/image/IOSS`)
      .then((res) => {
    ...
    // HOW IT WORKS
    axios.get(`/api/v1/work/role/IOSS`)
      .then((res) => {
        console.log(res)
        setworks(res?.data)
      })
  ...
  return (
    <div className=' font-poppinsFont text-black m-3 relative '>
      <h1 className='text-right tracking-wider mb-4'>IOSS Page Content</h1>
      {
        iossTop.length === 0 ?
          <>
            <button onClick={() => {
            ...
            <div className=' flex justify-center items-center mt-0 mb-3'>
              <h1 className=' tracking-wider'>No Ioss Header Found Adds Header</h1>
            </div>
          </>
          : ""
      }
      ...
      {iossQuestion.length === 0 ?
        <>
          <div className=' flex justify-center items-center mt-0 mb-3'>
            <h1 className=' tracking-wider'>No Ioss Question Found</h1>
          </div>
        </>
        : ""}
      ...
      {issoImageQuestion.length === 0 ?
        <>
          <div className=' flex justify-center items-center mt-0 mb-3'>
            <h1 className=' tracking-wider'>No IossImage Question Found</h1>
          </div>
        </>
        : ""}

      <button onClick={() => nav('/C/IOSS')} className=' bg-gray-700 text-white h-[2.5rem] w-[11rem] rounded-md text-sm hover:bg-blue-600 mb-4'>Add Image Question</button>
      <div>
        {
      ...
      <button onClick={() => {nav('/work/create/IOSS')}} className=' bg-gray-700 text-white h-[2.5rem] w-[11rem] rounded-md text-sm hover:bg-blue-600 mb-4'>Add How It Works</button>
      <div>
        {
                ...
                return (
                  <div key={index} className=' flex items-center justify-center flex-col p-3 w-[26rem] h-[15rem] overflow-scroll shadow-shadow1 bg-slate-400 flex-shrink-0'>
                    ...
                    <div className='flex items-center gap-[19rem]'>
                      ...
                      <div className='cursor-pointer' onClick={() => nav(`/work/IOSS/${i._id}`)}>
